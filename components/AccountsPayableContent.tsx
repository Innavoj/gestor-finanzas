...
const AccountsPayableContent: React.FC = () => {
  ...
  const totalPayable = useMemo(() => filteredPayables.reduce((sum, t) => sum + t.amount, 0), [filteredPayables]);
  const totalOverdue = useMemo(() => filteredPayables.filter(t => t.isOverdue).reduce((sum, t) => sum + t.amount, 0), [filteredPayables]);
  ...
  const handleExport = () => {
    ...
    const dataForPdf = filteredPayables.map(t => ({
      ...t,
      isOverdue: checkOverdue(t.dueDate, t.status)
    }));
    exportToPdfUtil(columns, dataForPdf, 'Reporte_Cuentas_Por_Pagar');
  };

  if (state.isLoading && state.transactions.length === 0) {
    return <div className="p-6 text-center text-slate-500">Cargando cuentas por pagar...</div>;
  }
  if (state.error) {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 bg-lightgray min-h-screen">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center space-y-2 sm:space-y-0">
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-800">Cuentas por Pagar</h1>
        <button
          onClick={handleExport}
          disabled={filteredPayables.length === 0 || isSubmitting}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
        <StatCard title="Total por Pagar (Filtrado)" value={totalPayable} color="text-accent-negative" />
       ...
       <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
          <label htmlFor="filterStatusAp" className="block text-sm font-medium text-slate-600">Filtrar por Estado:</label>
          <select 
            id="filterStatusAp" 
            value={filterStatus} 
      ...
      <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg overflow-x-auto">
        <h2 className="text-lg sm:text-xl font-semibold text-slate-700 mb-4">Lista de Cuentas por Pagar ({filteredPayables.length})</h2>
        {filteredPayables.length > 0 ? (
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {filteredPayables.map(t => (
                <tr key={t.id} className={`hover:bg-slate-50 transition-colors ${t.isOverdue ? 'bg-amber-50' : ''}`}>
                  ...
                  <td className="px-2 py-2 sm:px-4 sm:py-3 whitespace-nowrap text-xs sm:text-sm text-right font-medium text-accent-negative">
          ...
          <p className="text-slate-500 text-center py-4">No hay cuentas por pagar que coincidan con los filtros.</p>
        )}
      </div>
      ...
      {showMarkAsPaidModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-4 sm:p-6 rounded-lg shadow-xl w-full max-w-xs sm:max-w-md">
            ...
            <div>
              <label htmlFor="paymentDateModal" className="block text-xs sm:text-sm font-medium text-slate-700 mb-1">Fecha de Pago:</label>
              <input 
                type="date" 
                id="paymentDateModal" 
                value={paymentDate} 
